...
const sendNotice = async () => {
  ...
  try {
    ...
    setTimeout(() => { showNoticeModal.value = false; noticeResult.value = '' }, 1200)
  } catch {
      ...
      <div class="content">
        ...
        <div class="filter_row">
          ...
          <button class="search_btn" @click="fetchList">검색</button>
            <button class="notice_btn" @click="openNoticeModal">
                라이더 공지 발송
            </button>
        </div>
            ...
            <tbody>
              <template v-if="filteredList.length > 0">
                <tr v-for="item in filteredList" :key="item.deliveryNo">
                  ...
                  <td>
                    <a :href="`tel:${ADMIN_TEL}`" class="tel_link" :title="ADMIN_TEL">
    ...
    <div v-if="showNoticeModal" class="modal_overlay" @click.self="closeNoticeModal">
      <div class="modal">
        ...
        <div class="modal_body">
          ...
          <div class="form_group">
            ...
            <div class="target_row">
              <label class="radio_label">
                <input type="radio" v-model="noticeForm.targetType" value="ALL" /> 전체
              </label>
              <label class="radio_label">
                <input type="radio" v-model="noticeForm.targetType" value="SPECIFIC" /> 특정 구역
              </label>
            </div>
          ...
          <div class="form_two_col">
            ...
            <div class="form_group">
              ...
              <div class="sendtype_col">
                <label class="radio_label">
                  <input type="radio" v-model="noticeForm.sendType" value="NOW" /> 즉시 전송
                </label>
                <label class="radio_label">
                  <input type="radio" v-model="noticeForm.sendType" value="RESERVED" /> 예약 전송
                </label>
          ...
          <p v-if="noticeResult" class="notice_result" :class="{ error: noticeResult.includes('실패') }">
            {{ noticeResult }}
          </p>
        </div>
...
.notice_btn { background: #9b1b1b; color: #fff; border: none; border-radius: 6px; padding: 6px 20px; font-size: 13px; font-weight: 600; cursor: pointer; margin-left:auto }
...
/* 필터 */
.filter_row { display: flex; align-items: center; gap: 12px; margin-top: 50px;}
...
.search_btn { background: #9b1b1b; color: #fff; border: none; border-radius: 6px; padding: 6px 18px; font-size: 13px; font-weight: 600; cursor: pointer; height: autopx; }
...
/* 전화 링크 */
.tel_link { font-size: 18px; color: #888; text-decoration: none; cursor: pointer; display: inline-block; }
.tel_link:hover { color: #4a90d9; }
